...
function renderProductDetail(p) {
  ...
  wrap.innerHTML =
    ...
    '<div class="product-detail-info">' +
      '<span class="product-category">' + escapeHtml(categoryLabels[p.category] || p.category) + '</span>' +
      '<h1>' + escapeHtml(p.name) + '</h1>' +
      '<span class="product-price">' + formatFCFA(p.price) + '</span>' +
      '<p class="product-desc">' + escapeHtml(p.description) + '</p>' +
      '<p class="product-stock ' + (outOfStock ? 'out' : 'in') + '">' +
        (outOfStock ? '<i class="fas fa-circle-xmark"></i> Rupture de stock' : '<i class="fas fa-circle-check"></i> En stock') +
      '</p>' +
...
function changeQty(delta) {
  ...
  var value = Math.min(max, Math.max(1, Number(input.value) + delta));
  input.value = value;
}
